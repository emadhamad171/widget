import { r as k } from "./index-X9yZCR4S.mjs";
import { am as w, an as I, ao as q } from "./index-DKNltDvP.mjs";
const v = () => "9.1.0", C = (t) => t.toString(16).padStart(2, "0"), R = (t) => {
  const e = new Uint8Array(t / 2);
  return window.crypto.getRandomValues(e), Array.from(e, C).join("");
}, E = () => typeof window < "u" ? R(10) : (/* @__PURE__ */ new Date()).getTime().toString(36);
class g {
}
g.makeRequest = (t, e) => ({
  id: E(),
...
class H {
  constructor(e = null, s = !1) {
    this.allowedOrigins = null, this.callbacks = /* @__PURE__ */ new Map(), this.debugMode = !1, this.isServer = typeof window > "u", this.isValidMessage = ({ origin: a, data: n, source: i }) => {
      ...
      let y = !0;
      return Array.isArray(this.allowedOrigins) && (y = this.allowedOrigins.find((P) => P.test(a)) !== void 0), !o && l && B && y;
...
const S = (t) => typeof t == "object" && t != null && "domain" in t && "types" in t && "message" in t;
var m = k();
class A {
  constructor(e) {
    this.communicator = e;
...
const x = "0x1626ba7e", V = "0x20c13b0b", p = 4001;
...
class _ {
  ...
  async requestPermissions(e) {
    if (!this.isPermissionRequestValid(e))
      throw new d("Permissions request is invalid", p);
    try {
      return (await this.communicator.send(r.wallet_requestPermissions, e)).data;
    } catch {
      throw new d("Permissions rejected", p);
    }
  }
  isPermissionRequestValid(e) {
    return e.every((s) => typeof s == "object" ? Object.keys(s).every((a) => !!Object.values(f).includes(a)) : !1);
  }
}
const b = (t, e) => e.some((s) => s.parentCapability === t), M = () => (t, e, s) => {
  const a = s.value;
  return s.value = async function() {
    const n = new _(this.communicator);
    let i = await n.getPermissions();
    if (b(e, i) || (i = await n.requestPermissions([{ [e]: {} }])), !b(e, i))
      throw new d("Permissions rejected", p);
    return a.apply(this);
  }, s;
};
...
class T {
  ...
  async check1271Signature(e, s = "0x") {
    const a = await this.getInfo(), n = w({
      abi: [
        {
          constant: !1,
          inputs: [
  ...
  async check1271SignatureBytes(e, s = "0x") {
    ...
    try {
      return (await this.communicator.send(r.rpcCall, i)).data.slice(0, 10).toLowerCase() === V;
    } catch {
      return !1;
    }
  }
...
j([
  M()
], T.prototype, "requestAddressBook", null);
class L {
  constructor(e = {}) {
    const { allowedDomains: s = null, debug: a = !1 } = e;
    this.communicator = new H(s, a), this.eth = new O(this.communicator), this.txs = new A(this.communicator), this.safe = new T(this.communicator), this.wallet = new _(this.communicator);
  }
}
const F = m.Operation, G = m.TokenType, U = m.TransactionStatus, K = m.TransferDirection;
export {
  g as MessageFormatter,
  r as Methods,
  F as Operation,
  c as RPC_CALLS,
  f as RestrictedMethods,
  G as TokenType,
  U as TransactionStatus,
  K as TransferDirection,
  L as default,
  v as getSDKVersion,
  S as isObjectEIP712TypedData
};
//# sourceMappingURL=index-2gmYURHu.mjs.map

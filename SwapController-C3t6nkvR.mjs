import { N as T, w as G, R as A, S as w, o as B, q as P, E as D, t as M, W as F, C as g, B as N, a as L, u as v, x as O, y as z, d as _, e as R, z as H, s as V, i as q, p as $ } from "./index-CmqJ8p7x.mjs";
const I = {
  ...
  getParams() {
    var i, u, p, f, S, h, y, C, b;
    const n = g.state.activeChain, t = ((i = g.getAccountData(n)) == null ? void 0 : i.caipAddress) ?? g.state.activeCaipAddress, o = L.getPlainAddress(t), s = R(), c = H.getConnectorId(g.state.activeChain);
    if (!o)
    ...
    return {
      ...
      toTokenDecimals: (C = e.toToken) == null ? void 0 : C.decimals,
      sourceTokenAmount: e.sourceTokenAmount,
      sourceTokenDecimals: (b = e.sourceToken) == null ? void 0 : b.decimals,
      invalidToToken: r,
    ...
    if (!e.initializing) {
      if (e.initializing = !0, !e.initialized)
        try {
          ...
          e.initialized = !1, w.showError("Failed to initialize swap"), A.goBack();
        }
      e.initializing = !1;
    }
  },
  ...
  async getTokenList() {
    var t, o;
    const n = (t = g.state.activeCaipNetwork) == null ? void 0 : t.caipNetworkId;
    if (!(e.caipNetworkId === n && e.tokens))
      try {
        e.tokensLoading = !0;
        const s = await v.getTokenList(n);
  ...
  async getAddressPrice(n) {
    ...
    if (t)
      return t;
    const o = await N.fetchTokenPrice({
      addresses: [n]
  ...
  async getNetworkTokenPrice() {
    var c;
    const { networkAddress: n } = a.getParams(), o = (c = (await N.fetchTokenPrice({
      addresses: [n]
    }).catch(() => (w.showError("Failed to fetch network token price"), { fungibles: [] }))).fungibles) == null ? void 0 : c[0], s = (o == null ? void 0 : o.price.toString()) || "0";
  ...
  async getMyTokensWithBalance(n) {
    var s;
    const t = await z.getMyTokensWithBalance({
      forceUpdate: n,
      caipNetwork: g.state.activeCaipNetwork,
      address: (s = g.getAccountData()) == null ? void 0 : s.address
    }), o = v.mapBalancesToSwapTokens(t);
    o && (await a.getInitialGasPrice(), a.setBalances(o));
  },
  setBalances(n) {
    const { networkAddress: t } = a.getParams(), o = g.state.activeCaipNetwork;
    if (!o)
      return;
  ...
  async getInitialGasPrice() {
    var t, o;
    const n = await v.fetchGasPrice();
    if (!n)
      return { gasPrice: null, gasPriceInUSD: null };
    switch ((o = (t = g.state) == null ? void 0 : t.activeCaipNetwork) == null ? void 0 : o.chainNamespace) {
      case P.CHAIN.SOLANA:
      ...
      case P.CHAIN.EVM:
      default:
        const s = n.standard ?? "0", c = BigInt(s), r = BigInt(x), d = I.getGasPriceInUSD(e.networkPrice, r, c);
  ...
  async swapTokens() {
    var r, d, l;
    const n = (r = g.getAccountData()) == null ? void 0 : r.address, t = e.sourceToken, o = e.toToken, s = T.bigNumber(e.sourceTokenAmount).gt(0);
    if (s || a.setToTokenAmount(""), !o || !t || e.loadingPrices || !s || !n)
      return;
    e.loadingQuote = !0;
    const c = T.bigNumber(e.sourceTokenAmount).times(10 ** t.decimals).round(0).toFixed(0);
    try {
      const i = await N.fetchSwapQuote({
        userAddress: n,
        from: t.address,
        to: o.address,
        gasPrice: e.gasFee,
      ...
      const u = await v.handleSwapError(i);
  ...
  async getTransaction() {
    ...
    if (!(!n || !t || !o || !s || e.loadingQuote))
      try {
        ...
        const c = await v.fetchSwapAllowance({
          userAddress: n,
        ...
        A.goBack(), w.showError("Failed to check allowance"), e.loadingBuildTransaction = !1, e.approvalTransaction = void 0, e.swapTransaction = void 0, e.fetchError = !0;
        return;
      }
  },
  async createAllowanceTransaction() {
    ...
    if (!(!n || !o)) {
      ...
      try {
        const s = await N.generateApproveCalldata({
          from: t,
          to: o,
          userAddress: n
        ...
        A.goBack(), w.showError("Failed to create approval transaction"), e.approvalTransaction = void 0, e.swapTransaction = void 0, e.fetchError = !0;
        return;
      }
    }
  },
  async createSwapTransaction() {
    ...
    try {
      const l = await N.generateSwapCalldata({
        userAddress: t,
        from: s.address,
        to: c.address,
        amount: r,
      ...
      A.goBack(), w.showError("Failed to create transaction"), e.approvalTransaction = void 0, e.swapTransaction = void 0, e.fetchError = !0;
      return;
    }
  },
  onEmbeddedWalletApprovalSuccess() {
    w.showLoading("Approve limit increase in your wallet"), A.replace("SwapPreview");
  },
  // -- Send Transactions --------------------------------- //
  async sendTransactionForApproval(n) {
    ...
    }) : w.showLoading("Approve limit increase in your wallet");
    try {
      ...
      const i = l;
      e.transactionError = i == null ? void 0 : i.displayMessage, e.loadingApprovalTransaction = !1, w.showError((i == null ? void 0 : i.displayMessage) || "Transaction error"), D.sendEvent({
        type: "track",
        event: "SWAP_APPROVAL_ERROR",
        properties: {
          message: (i == null ? void 0 : i.displayMessage) || (i == null ? void 0 : i.message) || "Unknown",
          network: ((c = g.state.activeCaipNetwork) == null ? void 0 : c.caipNetworkId) || "",
  ...
  async sendTransactionForSwap(n) {
    ...
    s ? A.pushTransactionStack({
      onSuccess() {
        A.replace("Account"), w.showLoading(c), E.resetState();
      }
    }) : w.showLoading("Confirm transaction in your wallet");
    try {
      ...
      return e.loadingTransaction = !1, w.showSuccess(r), D.sendEvent({
        type: "track",
        event: "SWAP_SUCCESS",
        properties: {
      ...
      const m = U;
      e.transactionError = m == null ? void 0 : m.displayMessage, e.loadingTransaction = !1, w.showError((m == null ? void 0 : m.displayMessage) || "Transaction error"), D.sendEvent({
        type: "track",
        event: "SWAP_ERROR",
        properties: {
  ...
  setTransactionDetails() {
    ...
    !n || !t || (e.gasPriceInUSD = I.getGasPriceInUSD(e.networkPrice, BigInt(e.gasFee), BigInt(x)), e.priceImpact = I.getPriceImpact({
      sourceTokenAmount: e.sourceTokenAmount,
...
//# sourceMappingURL=SwapController-C3t6nkvR.mjs.map
